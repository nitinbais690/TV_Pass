...
import { queryString } from 'components/qp-discovery-ui/src/utils/URLBuilder';
import { ResourceResponse } from 'components/qp-discovery-ui/src/models/Storefront.types';
import { metaDataResourceAdapter } from 'components/qp-discovery-ui/src/models/Adapters';
...
export const StorefrontViewallScreen = ({ route, navigation }: { route: any; navigation: any }): JSX.Element => {
    ...
    const cardsPerRow = selectDeviceType({ Handset: channelsCardsPerRow ? 3 : 2 }, 3);
    ...
    const defaultRenderResource = React.useCallback(
        ({ item }: { item: ResourceVm }): JSX.Element => {
            ...
            const collectionStyle = StyleSheet.create({
                ...
                wrapperStyle: {
                    width: width / cardsPerRow,
                    aspectRatio: viewAllAspectRatio && title === 'Channels' ? viewAllAspectRatio : aspectRatio,
    ...
    return (
        <ModalOverlay scrollY={scrollY} isCollapsable={true} headerTransparent={true} headerTitle={undefined}>
