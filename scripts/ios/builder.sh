...
# buildNumber=$(expr $(git rev-list master --count) - $(git rev-list HEAD..master --count))
# xcrun agvtool new-version -all ${BUILD_NUMBER}
# xcrun agvtool new-marketing-version ${BUILD_NAME}
cd $WORKING_DIR

echo "Archiving the project"
xcodebuild clean archive -quiet -workspace $cur_dir/../../ios/${PROJECT_NAME}.xcworkspace -scheme $IOS_SCHEME -configuration $IOS_CONFIGURATION -derivedDataPath $cur_dir/../../ios/build -archivePath $cur_dir/../../ios/build/Products/${PROJECT_NAME}.xcarchive
